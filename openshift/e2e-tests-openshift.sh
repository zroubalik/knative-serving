...
readonly KNATIVE_BUILD_VERSION="v0.5.0"
...
function install_knative(){
  ...
  #sed "/--install/a\ $(printf %25s)- --filename=${RELEASE_YAML}"  openshift/olm/knative-serving.catalogsource.yaml > knative-serving.catalogsource-ci.yaml
  ...
  oc apply -n $OLM_NAMESPACE -f https://raw.githubusercontent.com/openshift/knative-build/release-${KNATIVE_BUILD_VERSION}/openshift/olm/knative-build.catalogsource.yaml
  timeout 120 "oc get pods -n $OLM_NAMESPACE | grep knative"
  ...
  # Deploy Knative Operators for Build and Serving
  deploy_knative_operator build
...
function delete_knative_openshift() {
  ...
  oc delete --ignore-not-found=true  -n $OLM_NAMESPACE -f https://raw.githubusercontent.com/openshift/knative-build/release-${KNATIVE_BUILD_VERSION}/openshift/olm/knative-build.catalogsource.yaml
